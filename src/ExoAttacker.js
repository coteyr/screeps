...
* @Last Modified time: 2016-08-15 02:39:37
*/
  ...
  if (this.mode() !== 'move-out' && this.mode() !== 'transition') {
    ...
    if(flag) {
      if(Game.rooms[this.memory.exo_target] && this.hits >= this.hitsMax) {
        if (_.size(Game.rooms[this.memory.exo_target].find(FIND_MY_CREEPS)) >= ARMY[this.room.tactic()].rally) this.setMode('move-out')
      }
      if(_.size(flag.pos.findInRange(FIND_MY_CREEPS, 5)) >= ARMY[this.room.tactic()].rally) this.setMode('move-out')
    } else {
